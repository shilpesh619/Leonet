# leonet_train_pipeline.py (Fixed motor_output dim mismatch)
import torch
...
def generate_command_dataset(path="leonet_command_dataset.jsonl"):
    ...
    for _ in range(100):
        ...
        token_ids = tokens[:8] + [0] * max(0, 8 - len(tokens[:8]))  # Truncate or pad to length 8
        # Pad vector to 128 dimensions
        motor_vec = vector + [0.0] * (128 - len(vector))
        entry = {
            "input_ids": token_ids,
            "target_ids": token_ids,
            "motor_output": motor_vec * 8
        }
    ...
    with open(path, "w") as f:
        for item in dataset:
            json.dump(item, f)
            f.write("\n")
    print("✅ Auto-generated leonet_command_dataset.jsonl")
